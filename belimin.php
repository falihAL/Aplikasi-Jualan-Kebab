...
if (empty($_SESSION['orderanmin']) OR !isset($_SESSION['orderanmin'])) {
	echo "<script> alert('Belum ada yang order'); document.location.href='menu-minuman.php'</script>";
}
...
<table border="1">
	...
	<tbody>
		<?php 
			...
			$totalorder = 0;
		?>
		<?php foreach($_SESSION['orderanmin'] as $id_minum => $jumlah): ?>
		<?php
			$ambil = $konek->query("SELECT * FROM minuman WHERE id='$id_minum'");
			$pecah = $ambil->fetch_assoc();
			$_SESSION['minum'] = $pecah;
		...
		<tr>
			...
			<td bgcolor="#93baf9">
				<a href="hapus-order.php?id=<?php echo $id_minum ?>">Hapus</a>
			</td>
		</tr>
	<?php 
		$nomor++;
		$totalorder+=$total;
		$_SESSION['totalal'] = $totalorder;
	?>
	<?php endforeach ?>
	</tbody>
	<tfoot>
...
<form action="nota.php" method="post">
	...
	<input type="submit" name="cetakan" value="Cetak Nota">
</form>
</div>
</div>
